...
import br.com.fiap.entity.pk.PedidosPK;
import br.com.fiap.helper.VendasHelper;

public class TestaAplicacao {
	public static void main(String[] args) {
		EntityManagerFactory emf =
				Persistence.createEntityManagerFactory("SCJ Persistência Roteiro 5 JPA");
		...
		try {
			...
			//Fazendo as associac?o?es
			pedido.getItens().add(item1);
			pedido.getItens().add(item2);
			cliente.getEnderecos().add(endereco);
			cliente.getPedidos().add(pedido);
			dao.salvar(cliente);
			System.out.println("Cliente inclui?do com sucesso!");
		}
		catch (Exception e) {
			e.printStackTrace();
		}
